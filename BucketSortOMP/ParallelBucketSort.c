...
/*
 *	Parallel bucketsort algorithm implemented using OpenMP
 *	For performance see readme
 *
 */
void bucketSortP(int *const arr, const int size);

/*
 *	Sequential bucketsort implementation used for global sorting
 */
void bucketSortS(int *const arr, int *const auxiliaryArr, const int size,
...
#pragma omp barrier
    ...
    int bucketRange =
        LIMIT / numberOfLocalBuckets != 0 ? LIMIT / numberOfLocalBuckets : 1;
...
#pragma omp barrier
    ...
    for (size_t i = 0; i < numThreads; i++) {
      ...
      int numberOfElementsInLocalBucket =
